...
def get_data_format(data):
    ...
    for operation in data:
        ...
        currency_info = operation["operationAmount"]["currency"]

        if isinstance(currency_info, dict):
            currency_formatted = f"{currency_info['name']} ({currency_info['code']})"
        else:
            currency_formatted = str(currency_info)

        date_formatted = format_date(operation["date"])
        description = operation["description"]
        formatted_operation = f"{date_formatted} {description}\n{from_formatted} -> {to_formatted}\n{amount} {currency_formatted}\n"
